...
Usage:
     ...
     -src|--source               Source clip to be cut
     -st|--start-time               Time to start cut
     -et|--end-time            Time to end cut
     -o|--output-clip-name               Output Clip Name
...
function parse_params() {
    ...
    while [[ $# -gt 0 ]]; do
        ...
        -src | --source)
            source_clip="$1"
            # printf "\n source_clip param: $param \n 1:$1 \n 2:$2"
            ;;
        -st | --start-time)
            start_time="$1"
            # printf "\n start_time param: $param \n 1:$1 \n 2:$2"
            ;;
        -et | --end-time)
            end_time="$1"
            # printf "\n end_time param: $param \n 1:$1 \n 2:$2"
            ;;
        -o | --output-clip-name)
            # printf "\n output_name param: $param \n 1:$1 \n 2:$2"
            output_name="$1"
            ;;
        -v | --verbose)
            verbose=true
...
function main() {
    ...
    # outputDir="clips"
    #mkdir $outputDir
    #echo "where is this: `pwd`"
    #dir=`pwd`

    # for entry in $(ls -Sr *.{mp4,flv,ts,mkv}); do
    #     echo "$entry"
    #     start=$(date +%s)
    #     sizeOld=$(stat -c%s $entry)
    #     newName=${entry::-3}.mp4
    # ffmpeg -i "$source_clip" -ss "$start_time" -t "$end_time" -async 1 "$output_name" &&
    ffmpeg -ss "$start_time" -i "$source_clip" -to "$end_time" -c copy "$output_name" &&
        #  -i $entry -r 25 $outputDir/${newName}
        {
            # rm $entry
            # sizeNew=$(stat -c%s $outputDir/$newName)
            # end=$(date +%s)
            # runtime=$((end - start))
            printf "\n All ok"
        } ||
        {
            printf "\n Something happened while dealing with: $entry" >>conversion.log
        }
    # done
}
